...
public struct EnvVarHeaders {
    ...
    /// This resource information is loaded from the environment variable.
    public static let attributes: [(String, String)]? = EnvVarHeaders.attributes()

    public static func attributes(
        for rawEnvAttributes: String? = ProcessInfo.processInfo.environment["OTEL_EXPORTER_OTLP_HEADERS"]
    ) -> [(String, String)]? {
