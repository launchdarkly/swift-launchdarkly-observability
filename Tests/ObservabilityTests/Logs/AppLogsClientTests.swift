...
struct AppLogsClientTests {
    ...
    func theSameLevelEnabled() {
        ...
        for severity in OTelSeverities {
            guard let level = ObservabilityOptions.LogLevel(rawValue: severity.rawValue) else {
                continue
            }
            let sut = AppLogClient(
                logLevel: level,
...
final class LogsApiSpy: InternalLogsApi {
    ...
    func recordLog(message: String, severity: OpenTelemetryApi.Severity, attributes: [String : OpenTelemetryApi.AttributeValue], spanContext: SpanContext?) {
        invokeCount += 1
        guard let level = ObservabilityOptions.LogLevel(rawValue: severity.rawValue) else { return }
